...
/* ── Lucide icon renderer (passed as prop from Home) ── */

/* ══════════════════════════════════════
   EMOJI PICKER  (full-screen overlay)
══════════════════════════════════════ */
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {EMOJI_LIST.map(e => (
            <button
              ...
              onPointerOver={ev => ev.currentTarget.style.background = T.hover}
              onPointerOut={ev => ev.currentTarget.style.background = "transparent"}
            >
...
/* ══════════════════════════════════════
   FOLDER SHEET  (move doc to folder)
══════════════════════════════════════ */
        ...
        <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none" }}>
          {projects.map(proj => (
            <div key={proj.id}>
              ...
              {proj.folders.map(f => (
                <button key={f.id} onClick={() => onMove(f.id, proj.id)}
                  style={{
...
export function NewFolderDialog({ newFolderDlg, newFolderName, setNewFolderName, onConfirm, onClose, T, Icon }) {
  ...
  return (
    <>
      <div style={{ position: "fixed", inset: 0, zIndex: 299, background: "rgba(0,0,0,.35)" }} onClick={onClose} />
      <div style={{
        ...
        <div style={{ padding: "24px 24px 8px" }}>
          {/* Icon — no container div, just the icon directly */}
...
export function ThemeDialog({ theme, setTheme, onClose, T, Icon, isDark }) {
  ...
  return (
    <>
      <div style={{ position: "fixed", inset: 0, zIndex: 399, background: "rgba(0,0,0,.35)" }} onClick={onClose} />
      <div style={{
        position: "fixed", top: "50%", left: "50%",
        transform: "translate(-50%,-50%)",
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Header row — icon bare, no wrapper div */}
        ...
        {/* Textarea — grows upward via minHeight, no overly round borders */}
        <textarea
          ref={ref}
          autoFocus
          value={aiInput}
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Template list — cards separados, sem linha, bordas progressivas */}
        <div style={{
