// App.jsx — raiz, gere a navegação entre Home e Editor
// Em produção: importa Home de "./Home" e Editor de "./Editor"
// Aqui os componentes estão inlined para o sandbox funcionar
...
const COLORS=["#000000","#34322d","#5e5e5b","#858481","#d1d5db","#ffffff","#dc2626","#ea580c","#d97706","#65a30d","#16a34a","#0891b2","#2563eb","#7c3aed","#db2777","#fca5a5","#fdba74","#fcd34d","#86efac","#93c5fd","#c4b5fd","#f9a8d4"];
...
const DOCS_INIT = [
  ...
  { id:4, title:"Ideias para Newsletter", preview:"Tópicos possíveis: tendências do setor, entrevista com especialistas, dicas práticas.", date:"Ontem, 11:20", tag:"Marketing" },
  { id:5, title:"Manual de Onboarding", preview:"Guia completo para novos colaboradores: política interna, ferramentas e primeiros passos.", date:"2 Jan, 10:00", tag:"RH" },
];
...
.screen{position:absolute;inset:0;display:flex;flex-direction:column;font-family:'DM Sans',sans-serif;background:#f8f8f7;-webkit-user-select:none;user-select:none;overflow:hidden;transition:transform .35s cubic-bezier(.22,1,.36,1),opacity .35s}
.screen.slide-out{transform:translateX(-60px);opacity:0;pointer-events:none}
...
.drawer-mask{position:fixed;inset:0;background:rgba(0,0,0,.18);z-index:15}
...
.home-scroll{flex:1;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;padding:20px 16px 200px}
...
/* floating bars */
.ftb{position:fixed;left:50%;transform:translateX(-50%);bottom:max(14px,env(safe-area-inset-bottom,14px));width:min(96vw,480px);z-index:20}
.home-ftb{position:fixed;left:50%;transform:translateX(-50%);bottom:max(20px,env(safe-area-inset-bottom,20px));width:min(92vw,440px);z-index:20;display:flex;align-items:center;gap:10px}
.pill{height:54px;background:rgba(255,255,255,.97);border:1px solid rgba(0,0,0,.08);border-radius:9999px;box-shadow:0 4px 20px rgba(0,0,0,.10),0 1px 4px rgba(0,0,0,.05);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);display:flex;align-items:center;overflow:hidden;position:relative}
...
.pill-input{flex:1;height:54px;background:rgba(255,255,255,.97);border:1px solid rgba(0,0,0,.08);border-radius:9999px;box-shadow:0 4px 20px rgba(0,0,0,.10);backdrop-filter:blur(20px);display:flex;align-items:center;padding:0 16px;gap:10px;cursor:text}
.circ-btn{width:54px;height:54px;border-radius:50%;border:1px solid rgba(0,0,0,.08);background:rgba(255,255,255,.97);backdrop-filter:blur(20px);box-shadow:0 4px 20px rgba(0,0,0,.10);cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#5e5e5b;transition:transform .12s}
...
/* selection / popup menus */
.sm{position:fixed;z-index:9999;background:#1c1c1e;border-radius:14px;box-shadow:0 8px 32px rgba(0,0,0,.5);overflow:visible;pointer-events:auto}
...
.img-wrap.selected img{outline:2px solid #2563eb;outline-offset:2px}
...
.resize-handle{position:absolute;bottom:4px;right:4px;width:18px;height:18px;cursor:se-resize;z-index:6;opacity:.7}
...
.img-mi{display:flex;align-items:center;gap:10px;padding:11px 16px;color:#fff;font-size:13.5px;font-weight:500;font-family:'DM Sans',sans-serif;cursor:pointer;border:none;background:transparent;width:100%;text-align:left}
.img-mi:active{background:rgba(255,255,255,.08)}
.img-mi.danger{color:#ff6b6b}
.img-sep{height:1px;background:rgba(255,255,255,.10);margin:2px 0}
/* alert boxes */
...
.alert-icon{flex-shrink:0;width:20px;height:20px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;margin-top:1px}
...
/* doc cards */
.doc-card{background:#fff;border-radius:12px;box-shadow:0 1px 3px rgba(0,0,0,.06),0 2px 8px rgba(0,0,0,.04);padding:14px 16px;margin-bottom:10px;cursor:pointer;transition:transform .1s;-webkit-user-select:none;user-select:none}
...
.section-label{font-size:11px;font-weight:700;color:#858481;text-transform:uppercase;letter-spacing:.07em;margin-bottom:10px}
/* ai sheet */
.ai-overlay{position:fixed;inset:0;z-index:60;background:rgba(0,0,0,.35);backdrop-filter:blur(3px);animation:fadeIn .2s both}
.ai-sheet{position:fixed;bottom:0;left:0;right:0;background:#fff;border-radius:20px 20px 0 0;padding:20px 18px max(28px,env(safe-area-inset-bottom,28px));z-index:61;animation:slideUp .28s cubic-bezier(.22,1,.36,1) both}
/* ai dot */
...
const S=(o)=>Object.entries(o).reduce((a,[k,v])=>(a[k]=v,a),{});
...
const TbChip=({children,active,onMouseDown,onClick,style,"data-chip":dc})=>(
  ...
  return<><PH title="Cor do texto"/>
    <div style={{display:"grid",gridTemplateColumns:"repeat(8,1fr)",gap:5,padding:"10px 14px"}}>
      {colors.map(c=><div key={c} onMouseDown={e=>e.preventDefault()} onClick={()=>onApply(c)}
        style={{width:24,height:24,borderRadius:4,background:c,cursor:"pointer",border:c==="#ffffff"?"2px solid rgba(0,0,0,.15)":"2px solid transparent"}}/>)}
    </div>
...
function FontPopup({fonts,curFont,search,setSearch,preview,setPreview,onExpand,onApply}){
  ...
  return<>
    <div style={{display:"flex",alignItems:"center",borderBottom:"1px solid rgba(0,0,0,.08)"}}>
      ...
      <button onMouseDown={e=>e.preventDefault()} onClick={onExpand} style={{width:32,height:32,border:"none",background:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#858481",marginRight:8}}><Icon name="maximize-2" size={14}/></button>
    </div>
    ...
    <div className="noscroll" style={{maxHeight:190,overflowY:"auto"}}>
      {!search?groups.map((g,gi)=>{const gf=filtered.filter(f=>f.g===g);if(!gf.length)return null;
        ...
        style={{padding:"9px 14px",cursor:"pointer",border:"none",background:"transparent",width:"100%",textAlign:"left",fontFamily:"inherit",color:f.v===curFont?"#2563eb":"#34322d"}}>
    ...
    {preview&&<div style={{borderTop:"1px solid rgba(0,0,0,.08)",padding:"10px 14px 12px",background:"#fafafa"}}>
      <div style={{fontSize:10,fontWeight:700,color:"#858481",textTransform:"uppercase",marginBottom:4}}>{preview.l}</div>
...
function StylesPopup({exec,onClose}){
  const stls=[{l:"Parágrafo",c:()=>exec("formatBlock","<p>"),i:"pilcrow"},{l:"Título 1",c:()=>exec("formatBlock","<h1>"),i:"heading-1"},{l:"Título 2",c:()=>exec("formatBlock","<h2>"),i:"heading-2"},{l:"Título 3",c:()=>exec("formatBlock","<h3>"),i:"heading-3"},{l:"Citação",c:()=>exec("formatBlock","<blockquote>"),i:"quote"},{l:"Código",c:()=>exec("formatBlock","<pre>"),i:"code"}];
...
function InsertPopup({exec,onClose,insertImg,insertTable}){
  const alerts=[
    ...
    {id:"info",icon:"info",ch:"i",bg:"#3b82f6",cls:"alert-info",title:"Informação",text:"Uma nota informativa."},
  ...
  return<><PH title="Inserir"/>
    ...
    <div>
      ...
      {alerts.map(a=><PI key={a.id} icon={a.icon} label={a.title} onClick={()=>{
        exec("insertHTML",`<div class="alert-box ${a.cls}" contenteditable="true"><div class="alert-icon" style="background:${a.bg};color:#fff">${a.ch}</div><div><div class="alert-title">${a.title}</div><div class="alert-text">${a.text}</div></div></div><p></p>`);
        onClose();
      }}/>)}
    </div>
  </>;
  ...
  return<><PH title="Formatar"/>
    ...
    <div style={{borderBottom:"1px solid rgba(0,0,0,.08)"}}>
      {[["1.0","1"],["1.5","1.5"],["2.0","2"]].map(([lbl,val])=>(
        <PI key={val} icon="align-vertical-justify-start" label={`Espaçamento ${lbl}`} onClick={()=>{if(edRef.current)edRef.current.style.lineHeight=val;onClose();}}/>
      ))}
    </div>
    <PI icon="trash-2" label="Limpar formatação" danger onClick={()=>{exec("removeFormat");onClose();}}/>
  </>;
}
function ImageContextMenu({pos,onClose,onAction}){
  const items=[
  ...
  return(
    <div className="img-menu" style={{left:pos.x,top:pos.y}} onMouseDown={e=>e.stopPropagation()}>
      {items.map((item,i)=>item===null
        ?<div key={i} className="img-sep"/>
        :<button key={i} className={`img-mi${item.danger?" danger":""}`} onClick={()=>{onAction(item.action);onClose();}}>
          <Icon name={item.icon} size={15} style={{color:item.danger?"#ff6b6b":"rgba(255,255,255,.5)",flexShrink:0}}/>{item.label}
        </button>
      )}
    </div>
  );
...
/* ─── HOME SCREEN ───────────────────────────────────────── */
function HomeScreen({visible, docs, setDocs, onOpenEditor}){
  ...
  const doAI=async()=>{
    ...
    setAiLoading(true);
    try{
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,system:"Cria um documento curto em português. Responde APENAS com o conteúdo do documento.",messages:[{role:"user",content:p}]})});
      const d=await r.json();
      const content=d.content?.map(i=>i.text||"").join("")||"";
      const nd={id:Date.now(),title:p.slice(0,40),preview:content.slice(0,110)+"…",date:"Agora",tag:"IA",aiContent:content};
  ...
  return(
    <div className={`screen${visible?" slide-in":" slide-out"}`} style={{zIndex:1}}>
      {/* Drawer */}
      <div className={`drawer${drawerOpen?" open":""}`}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:10}}>
          {[{icon:"book",label:"Biblioteca"},{icon:"settings",label:"Definições"},{icon:"users",label:"Membros"}].map(item=>(
            <button key={item.label} className="drawer-item" onClick={()=>setDrawerOpen(false)}>
              <Icon name={item.icon} size={18} style={{color:"#5e5e5b",flexShrink:0}}/>{item.label}
            </button>
          ))}
        </div>
      </div>
      {drawerOpen&&<div className="drawer-mask" style={{zIndex:15}} onClick={()=>setDrawerOpen(false)}/>}
      ...
      <div className="topbar" style={{transform:drawerOpen?"translateX(110px)":"none",transition:"transform 400ms cubic-bezier(.22,1,.36,1)"}}>
        ...
        <div style={{position:"absolute",left:"50%",transform:"translateX(-50%)",fontSize:15,fontWeight:600,color:"#34322d"}}>Documentos</div>
      </div>

      {/* List */}
      <div className="home-scroll noscroll" style={{transform:drawerOpen?"translateX(110px)":"none",transition:"transform 400ms cubic-bezier(.22,1,.36,1)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,background:"#fff",borderRadius:12,boxShadow:"0 1px 3px rgba(0,0,0,.06)",padding:"10px 14px",marginBottom:20}}>
          ...
          <input placeholder="Pesquisar documentos…" style={{flex:1,border:"none",outline:"none",fontSize:14,color:"#34322d",background:"transparent",fontFamily:"inherit",WebkitUserSelect:"text",userSelect:"text"}}/>
        </div>
            ...
            <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                  ...
                  <button onClick={e=>e.stopPropagation()} style={{border:"none",background:"transparent",cursor:"pointer",padding:0,color:"#d1d5db",flexShrink:0}}><Icon name="more-horizontal" size={16}/></button>
                </div>
      ...
      {/* Bottom bar */}
      <div className="home-ftb" style={{transform:drawerOpen?"translateX(55px)":"translateX(-50%)",left:drawerOpen?"calc(50% + 0px)":"50%",transition:"transform 400ms cubic-bezier(.22,1,.36,1)"}}>
        ...
        <>
          ...
          <div className="ai-sheet">
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
              ...
              <button onClick={()=>setAiOpen(false)} style={{border:"none",background:"transparent",cursor:"pointer",color:"#858481",padding:4}}><Icon name="x" size={18}/></button>
            </div>
            <textarea ref={aiRef} value={aiInput} onChange={e=>setAiInput(e.target.value)}
              placeholder="Ex: Relatório mensal de vendas com análise de resultados…" rows={4}
...
function EditorScreen({visible, doc, onBack}){
  ...
  useEffect(()=>{if(edRef.current&&doc?.aiContent&&visible){edRef.current.innerText=doc.aiContent;}},[doc,visible]);
  ...
  const tryShowSel=useCallback(()=>{
    ...
    const spaceAbove=r.top-GAP-AH-MH,spaceBelow=vh-r.bottom-GAP-AH-MH;
    let top,dir;
  ...
  const handlePointerDown=useCallback((e)=>{
    ...
    longPressTimer.current=setTimeout(()=>{
      ...
      const spaceBelow=vh-my-GAP-AH-MH;
      let top,dir;
      if(spaceBelow>=0){top=my+GAP+AH;dir="up";}else{top=Math.max(8,my-MH-GAP-AH);dir="down";}
  ...
  const wrapImage=useCallback((img)=>{
    ...
    wrap.addEventListener("click",e=>{e.stopPropagation();e.preventDefault();haptic("light");document.querySelectorAll(".img-wrap.selected").forEach(w=>{if(w!==wrap){w.classList.remove("selected");w.querySelector(".img-dots-btn").style.display="none";w.querySelector(".resize-handle").style.display="none";}});wrap.classList.add("selected");btn.style.display="flex";rh.style.display="flex";});
  ...
  return(
    <div className={`screen${visible?" slide-in":" offright"}`} style={{zIndex:2}}>
      {/* Topbar */}
      <div className="topbar">
        <button onClick={onBack} style={{width:44,height:44,border:"none",background:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:10,color:"#5e5e5b"}}>
        ...
        <div style={{position:"absolute",right:6,display:"flex",gap:2}}>
          {[["undo-2","undo"],["redo-2","redo"]].map(([ic,cmd])=>(
            <button key={cmd} onClick={()=>exec(cmd)} style={{width:44,height:44,border:"none",background:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:10,color:"#5e5e5b"}}><Icon name={ic} size={20}/></button>
          ))}
        </div>
      </div>
        ...
        <div className={`pill${aiMode?" ai":""}`}>
          {aiMode?(
            ...
            <div className="tbtrack noscroll">
              ...
              <Div/>
              <TbBtn onMouseDown={e=>e.preventDefault()} onClick={()=>setAiMode(true)} style={{background:"#eff6ff",color:"#2563eb"}}><Icon name="bot" size={17}/></TbBtn>
            </div>
          )}
      ...
      {/* Selection menu */}
      {selVisible&&(
        <div className={`sm${selPos.dir==="down"?" adown":" aup"} show`} style={{left:selPos.left,top:selPos.top,width:selPos.mw,"--ax":selPos.ax+"px"}} onMouseDown={e=>e.preventDefault()}>
      ...
      {/* Empty line long-press menu */}
      {emptyMenu&&(
        <div className={`sm${emptyMenu.dir==="down"?" adown":" aup"} show`} style={{left:emptyMenu.left,top:emptyMenu.top,width:emptyMenu.mw,"--ax":emptyMenu.ax+"px"}} onMouseDown={e=>e.preventDefault()}>
          <div className="sm-inner noscroll">
      ...
      {/* Image menu */}
      {imgMenu&&(<><div style={{position:"fixed",inset:0,zIndex:9998}} onClick={()=>setImgMenu(null)}/><ImageContextMenu pos={imgMenu.pos} onClose={()=>setImgMenu(null)} onAction={action=>handleImgAction(action,imgMenu.el,imgMenu.img)}/></>)}
        ...
        <div className="ff">
          <div style={{height:54,display:"flex",alignItems:"center",gap:10,padding:"0 14px",borderBottom:"1px solid rgba(0,0,0,.08)",flexShrink:0}}>
            ...
            <input value={ffSearch} onChange={e=>setFfSearch(e.target.value)} placeholder="Pesquisar fonte…" onMouseDown={e=>e.stopPropagation()}
              style={{flex:1,height:34,border:"1.5px solid rgba(0,0,0,.08)",borderRadius:9999,padding:"0 14px",fontSize:13,fontFamily:"inherit",outline:"none",background:"#f8f8f7",WebkitUserSelect:"text",userSelect:"text"}}/>
          ...
          <div className="noscroll" style={{display:"flex",gap:6,padding:"10px 14px",borderBottom:"1px solid rgba(0,0,0,.08)",overflowX:"auto",flexShrink:0}}>
            {["Todas",...new Set(FONTS.map(f=>f.g))].map(cat=>(
              <button key={cat} onClick={()=>setFfCat(cat)} style={{flexShrink:0,padding:"5px 13px",border:`1.5px solid ${ffCat===cat?"#2563eb":"rgba(0,0,0,.08)"}`,borderRadius:9999,fontFamily:"inherit",fontSize:12,fontWeight:600,cursor:"pointer",background:ffCat===cat?"#eff6ff":"transparent",color:ffCat===cat?"#2563eb":"#5e5e5b"}}>{cat}</button>
            ))}
          </div>
          <div className="noscroll" style={{flex:1,overflowY:"auto",display:"grid",gap:8,padding:14,gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))"}}>
            {FONTS.filter(f=>(!ffSearch||f.l.toLowerCase().includes(ffSearch.toLowerCase()))&&(ffCat==="Todas"||f.g===ffCat)).map(f=>(
              <div key={f.v} onClick={()=>setFfSel(f.v)} style={{border:`1.5px solid ${ffSel===f.v?"#2563eb":"rgba(0,0,0,.08)"}`,borderRadius:10,padding:"10px 12px",cursor:"pointer",background:ffSel===f.v?"#eff6ff":"#fff"}}>
                <div style={{fontSize:10,fontWeight:700,color:"#858481",textTransform:"uppercase",marginBottom:4}}>{f.g}</div>
...
export default function App(){
  ...
  return(
    <div style={{position:"relative",width:"100%",height:"100%",overflow:"hidden"}}>
      <HomeScreen visible={screen==="home"} docs={docs} setDocs={setDocs} onOpenEditor={openEditor}/>
