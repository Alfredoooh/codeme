...
//
// NOTA: qr_flutter precisa ser adicionado ao pubspec.yaml (ver YAML
// completo ao final).
...
class QrcodeTool {
  ...
  static Future<ToolResult> generate(Map<String, dynamic> input) async {
    ...
    final double size = ((input['size'] as num?)?.toDouble() ?? 400).clamp(100, 2000);
    final Color fgColor = _parseHexColor(input['foreground_color'] as String?) ?? Colors.black;
    final Color bgColor = _parseHexColor(input['background_color'] as String?) ?? Colors.white;
    final Color? gradientColor = _parseHexColor(input['gradient_color'] as String?);
    final String eyeStyle = (input['eye_style'] as String?) ?? 'square';
    final String moduleStyle = (input['module_style'] as String?) ?? 'square';
    ...
    try {
      final qrValidationResult = QrValidator.validate(
        ...
        errorCorrectionLevel: logoBase64 != null ? QrErrorCorrectLevel.H : QrErrorCorrectLevel.M,
      );

      if (qrValidationResult.status != QrValidationStatus.valid) {
        return ToolResult.error('Conteúdo inválido para gerar QR Code.', code: 'INVALID_INPUT');
      }
      ...
      return ToolResult.error('Erro ao gerar QR Code: $e', code: 'GENERATION_ERROR');
    }
  }
  ...
  static Future<Uint8List> _paintCustomQr({
    ...
    // Fundo
    canvas.drawRect(Rect.fromLTWH(0, 0, size, size), Paint()..color = bgColor);
    ...
    bool _isInsideAnyEye(int x, int y) {
      for (final eyePos in eyePositions) {
        if (x >= eyePos.dx && x < eyePos.dx + 7 && y >= eyePos.dy && y < eyePos.dy + 7) {
          return true;
        }
      }
      return false;
    ...
    for (final eyePos in eyePositions) {
      _drawEye(canvas, eyePos, cellSize, paint, eyeStyle);
    }
  ...
  static void _drawModule(Canvas canvas, Rect rect, Paint paint, String style) {
    switch (style) {
      case 'dot':
      ...
      case 'rounded':
        canvas.drawRRect(
          RRect.fromRectAndRadius(rect.deflate(rect.width * 0.05), Radius.circular(rect.width * 0.3)),
          paint,
        );
        break;
      case 'square':
  ...
  static void _drawEye(Canvas canvas, Offset moduleOffset, double cellSize, Paint paint, String style) {
    ...
    switch (style) {
      case 'circle':
        ...
        canvas.drawOval(innerRect.deflate(cellSize * 0.2), Paint()..color = Colors.white);
        canvas.drawOval(innerRect.deflate(cellSize * 1.2), paint);
        break;
      case 'rounded':
        canvas.drawRRect(RRect.fromRectAndRadius(outerRect, Radius.circular(cellSize * 2)), paint);
        canvas.drawRRect(
          RRect.fromRectAndRadius(innerRect.deflate(cellSize * 0.2), Radius.circular(cellSize * 1.5)),
          Paint()..color = Colors.white,
        );
        canvas.drawRRect(
          RRect.fromRectAndRadius(innerRect.deflate(cellSize * 1.2), Radius.circular(cellSize)),
          paint,
        );
        break;
      case 'square':
      default:
        ...
        canvas.drawRect(innerRect.deflate(cellSize * 0.2), Paint()..color = Colors.white);
