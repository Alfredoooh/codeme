...
async function generateToken(payload, secret, env) {
  ...
  if (env && env.IPC_USERS) {
    await env.IPC_USERS.put("session:" + jti, JSON.stringify({ userId: payload.id, createdAt: Date.now() }), { expirationTtl: 30 * 24 * 60 * 60 });
  }
  return token;
}
...
async function verifyFirebaseToken(idToken, projectId) {
  try {
    ...
    const publicKeys = await fetch("https://www.googleapis.com/robot/v1/metadata/x509/securetoken@system.gserviceaccount.com", { signal: AbortSignal.timeout(8000) }).then(r => r.json());
    const certPem = publicKeys[header.kid];
    if (!certPem) return null;
    const certCleaned = certPem.replace(/-----BEGIN CERTIFICATE-----/, "").replace(/-----END CERTIFICATE-----/, "").replace(/\n/g, "").trim();
    const certDer = Uint8Array.from(atob(certCleaned), c => c.charCodeAt(0));
    const spkiKey = extractSpkiFromCert(certDer);
    if (!spkiKey) return null;
    const cryptoKey = await crypto.subtle.importKey("spki", spkiKey, { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, false, ["verify"]);
    const signingInput = parts[0] + "." + parts[1];
    const sigBytes = Uint8Array.from(atob(parts[2].replace(/-/g, "+").replace(/_/g, "/")), c => c.charCodeAt(0));
    const valid = await crypto.subtle.verify("RSASSA-PKCS1-v1_5", cryptoKey, sigBytes, new TextEncoder().encode(signingInput));
    if (!valid) return null;
    return { uid: payload.sub, email: payload.email || null, name: payload.name || null, picture: payload.picture || null, provider: (payload.firebase && payload.firebase.sign_in_provider) || "unknown" };
  } catch (e) { console.error("[FIREBASE VERIFY ERROR]", e.message); return null; }
}
function extractSpkiFromCert(certDer) {
  try {
    ...
    function skipTag(buf, off) { off++; const r = readLength(buf, off); return r.next + r.len; }
    function enterSequence(buf, off) { if (buf[off] !== 0x30) return null; off++; const r = readLength(buf, off); return r.next; }
    let pos = enterSequence(certDer, 0);
    if (pos === null) return null;
    pos = enterSequence(certDer, pos);
    if (pos === null) return null;
    if (certDer[pos] === 0xa0) pos = skipTag(certDer, pos);
    pos = skipTag(certDer, pos); pos = skipTag(certDer, pos); pos = skipTag(certDer, pos); pos = skipTag(certDer, pos); pos = skipTag(certDer, pos);
    if (certDer[pos] !== 0x30) return null;
    const spkiStart = pos; pos++;
    const r = readLength(certDer, pos);
    return certDer.slice(spkiStart, r.next + r.len).buffer;
  } catch (e) { console.error("[SPKI EXTRACT ERROR]", e.message); return null; }
}
...
async function groqChat(apiKey, messages, model, systemPrompt, language) {
  const sysContent = systemPrompt && systemPrompt.trim().length > 0 ? systemPrompt : buildSystemInstruction(language || "pt", "");
  const allMessages = [{ role: "system", content: sysContent }, ...messages.filter(m => m.role !== "system")];
  return fetch(GROQ_BASE + "/chat/completions", { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + apiKey }, body: JSON.stringify({ model: model || "llama-3.3-70b-versatile", messages: allMessages, max_tokens: 8192, temperature: 0.7, stream: false }) });
}
async function groqChatStream(apiKey, messages, model, systemPrompt, language) {
  const sysContent = systemPrompt && systemPrompt.trim().length > 0 ? systemPrompt : buildSystemInstruction(language || "pt", "");
  const allMessages = [{ role: "system", content: sysContent }, ...messages.filter(m => m.role !== "system")];
  return fetch(GROQ_BASE + "/chat/completions", { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + apiKey }, body: JSON.stringify({ model: model || "llama-3.3-70b-versatile", messages: allMessages, max_tokens: 8192, temperature: 0.7, stream: true }) });
}
...
// ═══ TOOLS — proxy puro para o servidor externo. Nenhuma lógica
// de tool vive aqui; o worker só repassa e guarda a URL. ═══
...
async function handleListEvents(request, env) {
  ...
  const all = await Promise.all(ids.map(async id => { const data = await env.IPC_USERS.get("event:" + id); return data ? JSON.parse(data) : null; }));
...
async function handleDeleteEvent(request, env) {
  ...
  const raw = await env.IPC_USERS.get("events:" + payload.id);
  const ids = raw ? JSON.parse(raw) : [];
  await env.IPC_USERS.put("events:" + payload.id, JSON.stringify(ids.filter(i => i !== id)));
...
async function handleRegister(request, env) {
  ...
  const user = { id, name, email: email.toLowerCase(), passwordHash, avatar: null, provider: "email", firebaseUid: null, credits: FREE_CREDITS, isAdmin: false, blocked: false, preferences: { language: "pt", theme: "system", fontSize: "medium" }, profile: { age: body.age || null, country: body.country || null, state: body.state || null, city: body.city || null, occupation: body.occupation || null, occupationDetail: body.occupationDetail || null, bio: null }, stats: { totalConversations: 0, totalMessages: 0 }, createdAt: Date.now() };
...
async function handleLogout(request, env) {
  ...
  if (auth.startsWith("Bearer ")) {
    ...
    const parts = token.split(".");
    if (parts.length === 3) {
      try { const payload = JSON.parse(atob(parts[1])); if (payload.jti) await env.IPC_USERS.delete("session:" + payload.jti); } catch (e) {}
    }
  }
  return json({ success: true });
}
...
async function handleFirebaseAuth(request, env) {
  ...
  if (!userId) {
    ...
    if (!userId) {
      ...
      const user = { id: userId, name: firebaseUser.name || firebaseUser.email || "Utilizador Nexa", email: firebaseUser.email ? firebaseUser.email.toLowerCase() : null, passwordHash: null, avatar: firebaseUser.picture || null, provider: firebaseUser.provider, firebaseUid: uid, credits: FREE_CREDITS, isAdmin: false, blocked: false, preferences: { language: "pt", theme: "system", fontSize: "medium" }, profile: { age: null, country: null, state: null, city: null, occupation: null, occupationDetail: null, bio: null }, stats: { totalConversations: 0, totalMessages: 0 }, createdAt: Date.now() };
...
async function handleGetMe(request, env) {
  ...
  if (!userData) return error("Utilizador não encontrado", 404);
  const user = JSON.parse(userData);
  return json({ id: user.id, name: user.name, email: user.email, avatar: user.avatar || null, provider: user.provider || "email", credits: user.credits ?? FREE_CREDITS, preferences: user.preferences || {}, profile: user.profile || {}, isAdmin: !!user.isAdmin, stats: user.stats || {}, createdAt: user.createdAt });
}
async function handleUpdateMe(request, env) {
  ...
  if (body.password) { if (body.password.length < 6) return error("Password deve ter pelo menos 6 caracteres"); user.passwordHash = await hashPassword(body.password); }
...
async function handleListConversations(request, env) {
  ...
  const ids = raw ? JSON.parse(raw) : [];
  const all = await Promise.all(ids.map(async id => { const data = await env.IPC_USERS.get("conv:" + id); return data ? JSON.parse(data) : null; }));
...
async function handleDeleteAllConversations(request, env) {
  ...
  const raw = await env.IPC_USERS.get("convs:" + payload.id);
  const ids = raw ? JSON.parse(raw) : [];
  await Promise.all(ids.map(id => env.IPC_USERS.delete("conv:" + id)));
  await env.IPC_USERS.put("convs:" + payload.id, JSON.stringify([]));
  const userData = await env.IPC_USERS.get("user:" + payload.id);
  if (userData) { const user = JSON.parse(userData); if (user.stats) user.stats.totalConversations = 0; await env.IPC_USERS.put("user:" + payload.id, JSON.stringify(user)); }
...
async function handleAiTranscribe(request, env) {
  ...
  const groqRes = await fetch(GROQ_BASE + "/audio/transcriptions", { method: "POST", headers: { "Authorization": "Bearer " + env.GROQ_API_KEY }, body: outForm });
  if (!groqRes.ok) { const errText = await groqRes.text(); return error("Erro Groq Whisper: " + errText, groqRes.status); }
...
async function handleCreditsWebhook(request, env) {
  ...
  if (listRes.keys && listRes.keys.length > 0) {
    if (eventUserId) {
      for (const k of listRes.keys) {
        const raw = await env.IPC_USERS.get(k.name);
        if (!raw) continue;
  ...
  if (userDataRaw) { const user = JSON.parse(userDataRaw); user.credits = (user.credits ?? 0) + pending.credits; await env.IPC_USERS.put("user:" + pending.userId, JSON.stringify(user)); }
...
async function handleAdminListUsers(request, env) {
  ...
  const users = await Promise.all(ids.map(async id => {
    ...
    return { id: u.id, name: u.name, email: u.email, avatar: u.avatar || null, credits: u.credits ?? 0, isAdmin: !!u.isAdmin, blocked: !!u.blocked, profile: u.profile || {}, stats: u.stats || {}, createdAt: u.createdAt, preferences: u.preferences || {} };
  }));
...
async function handleAdminGetUser(request, env) {
  ...
  const u = JSON.parse(raw);
  return json({ id: u.id, name: u.name, email: u.email, avatar: u.avatar || null, provider: u.provider, credits: u.credits ?? 0, isAdmin: !!u.isAdmin, blocked: !!u.blocked, preferences: u.preferences || {}, profile: u.profile || {}, stats: u.stats || {}, createdAt: u.createdAt });
}
async function handleAdminUpdateUser(request, env) {
  ...
  if (body.blocked !== undefined) { u.blocked = !!body.blocked; if (u.blocked) await env.IPC_USERS.put("session_epoch:" + id, String(Date.now())); }
  if (body.password !== undefined && body.password.length >= 6) u.passwordHash = await hashPassword(body.password);
  if (body.email !== undefined) {
    const newEmail = String(body.email).toLowerCase().trim();
    if (newEmail !== u.email) { if (u.email) await env.IPC_USERS.delete("email:" + u.email); await env.IPC_USERS.put("email:" + newEmail, id); u.email = newEmail; }
  }
  ...
  return json({ id: u.id, name: u.name, email: u.email, avatar: u.avatar || null, credits: u.credits, isAdmin: !!u.isAdmin, blocked: !!u.blocked, preferences: u.preferences || {}, profile: u.profile || {}, stats: u.stats || {}, createdAt: u.createdAt });
}
async function handleAdminDeleteUser(request, env) {
  ...
  await Promise.all(convIds.map(cid => env.IPC_USERS.delete("conv:" + cid)));
...
async function handleAdminUserConversations(request, env) {
  ...
  const ids = raw ? JSON.parse(raw) : [];
  const all = await Promise.all(ids.map(async cid => { const data = await env.IPC_USERS.get("conv:" + cid); return data ? JSON.parse(data) : null; }));
...
async function handleAdminNotify(request, env) {
  ...
  if (Array.isArray(body.userIds) && body.userIds.length > 0) {
    const users = await Promise.all(body.userIds.map(async id => { const raw = await env.IPC_USERS.get("user:" + id); return raw ? JSON.parse(raw) : null; }));
